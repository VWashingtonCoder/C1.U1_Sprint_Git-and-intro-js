...
function removeArtist(array, index) {
   const artistRemoved = array.splice(index,1);
   return array.length;
}
...
function addArtist(array) { 
   array.push({id: 20, name: 'Your Name Here', years: 'Your Birth Year - current day', genre: 'Web Design', nationality: 
   'Your Nationality Here', bio: 'lorem ipsum'});
   return array;
}
...
function artistByCountry(array, nationality){
  ...
  for (let i = 0; i<array.length; i++){
    if(array[i].nationality === nationality){
      artistNation.push(array[i].name);
    }
  }
  return artistNation;
}
